...
internal class TcpServer
{
    ...
    /// 已经连接的对象池
    /// </summary>
    internal ConcurrentDictionary<int, ConnectClient> connectClient = null!;
    /// <summary>
    /// 客户端列表
    /// </summary>
    internal ConcurrentDictionary<int, string> clientList = null!;
    /// <summary>
    /// 发送线程数
    /// </summary>
    ...
    /// 需要发送的数据
    /// </summary>
    private ConcurrentQueue<SendingQueue>[] sendQueues = null!;
    /// <summary>
    /// 锁
    /// </summary>
    ...
    private void ProcessReceive(SocketAsyncEventArgs e)
    {
        ...
        if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
        {
            ...
            if (!connectClient.TryGetValue(connectId, out ConnectClient? client))
            {
                return;
            }
            //如果接收到数据,超时记录设置为0
    ...
    private void StartSend(object? thread)
    {
        while (true)
        {
            if (thread != null && sendQueues[(int)thread].TryDequeue(out SendingQueue? sending))
            {
                Send(sending);
            }
            else
    ...
    {
        if (sendQuere == null || !connectClient.TryGetValue(sendQuere.connectId, out ConnectClient? client))
        {
            return;
        }
        //如果发送池为空时,临时新建一个放入池中
    ...
    private void CloseClientSocket(SocketAsyncEventArgs e)
    {
        if (e.LastOperation == SocketAsyncOperation.Receive)
        {
            ...
            if (client.socket.Connected == false)
            {
                ...
                clientList.TryRemove(connectId, out string? clientip);
                return;
            }
            try
            {
            ...
            connectClient.TryRemove(connectId, out client);
            clientList.TryRemove(connectId, out string? clientip1);
            client = null;
        }
    }
