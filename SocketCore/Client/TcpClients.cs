...
internal class TcpClients
{
    ...
    /// 接收对象
    /// </summary>
    private SocketAsyncEventArgs receiveSocketAsyncEventArgs = null!;
    /// <summary>
    /// 发送对象默认数
    /// </summary>
    ...
    /// 需要发送的数据
    /// </summary>
    private ConcurrentQueue<SendingQueue> sendQueue = null!;
    /// <summary>
    /// 连接成功事件
    /// </summary>
    ...
    {
        if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success && e.Buffer != null)
        {
    ...
    private void StartSend()
    {
        while (true)
        {
            if (sendQueue.TryDequeue(out SendingQueue? sending))
            {
                Send(sending);
            }
            else
